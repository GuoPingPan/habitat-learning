...
def main():
    ...
    class MoveForwardAndSpin(habitat_sim.SceneNodeControl):
        def __call__(
            ...
            rotation = habitat_sim.utils.quat_rotate_vector()

            # 其中 scene_node.absolute_transformation().rotation_scaling()是根据 spin_amount: float得到转动角
            # spin_amount=45就是左转45度，

            print(f'rotation_scaling\n : {np.array(scene_node.absolute_transformation().rotation_scaling())}')

            print(f'forward_ax : {forward_ax}')
...
def strafe_action():
    ...
    def _strafe_impl(
        ...
        print(f'rotation_scaling : {np.array(scene_node.absolute_transformation().rotation_scaling())}')
        print(f'forward_ax : {forward_ax}')
